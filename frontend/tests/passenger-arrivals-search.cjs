...
(async () => {
  ...
  stops[4].forecast.reasons = [{ feature: 'current_delay', description: 'Delay already carried into this section', minutes: 4.2 }, { feature: 'recovery_minutes', description: 'Recovery allowance ahead', minutes: -1.5 }];
  stops[2].forecast.reasons = [{ feature: 'rain_mm', description: 'Reported seasonal rain context', minutes: 2.1 }];
  ...
  try {
    ...
    for (const width of [1440, 390, 320]) {
      ...
      assert.match(await page.locator('.pj-reasons').innerText(), /Delay already carried/);
      assert.match(await page.locator('.pj-reasons').innerText(), /Earlier estimate/);
      await page.getByRole('combobox', { name: 'Your arrival station' }).selectOption('2');
      assert.match(await page.locator('.pj-reasons').innerText(), /seasonal rain/);
      assert.doesNotMatch(await page.locator('.pj-reasons').innerText(), /Delay already carried/);
      await page.getByRole('combobox', { name: 'Your arrival station' }).selectOption('1');
      assert.equal(await page.locator('.pj-reasons').count(), 0, 'Actual arrivals do not get model explanations presented as causes');
